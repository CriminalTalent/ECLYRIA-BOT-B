...
class BattleEngine
  ...
  def start_1v1(user1_id, user2_id, reply_status)
    ...
    if BattleState.battle_of(user1_id) || BattleState.battle_of(user2_id)
      @mastodon_client.reply(reply_status, "이미 전투 중인 참가자가 있습니다.")
      return
    end
    ...
    battle_reply_status = result || reply_status
    battle_id =
      if battle_reply_status.is_a?(Hash) && battle_reply_status[:id]
        battle_reply_status[:id].to_s
      else
        "battle_#{user1_id}_#{user2_id}_#{Time.now.to_i}_#{rand(1000)}"
      end

    puts "[디버그] start_1v1: turn_order=#{turn_order.inspect}"
    puts "[디버그] start_1v1: current_turn=#{turn_order[0]} (#{turn_order[0].class})"
    puts "[디버그] start_1v1: battle_id=#{battle_id}"
    ...
    state = {
      ...
      reply_status: battle_reply_status,
      battle_id: battle_id
    }
  ...
  def start_2v2(user1_id, user2_id, user3_id, user4_id, reply_status)
    ...
    battle_id =
      if battle_reply_status.is_a?(Hash) && battle_reply_status[:id]
        ...
        "battle_#{ids.join('_')}_#{Time.now.to_i}_#{rand(1000)}"
      end
  ...
  def start_dummy_battle(user_id, difficulty, reply_status)
    # 이미 전투 중인 참가자 검사
    if BattleState.battle_of(user_id)
      @mastodon_client.reply(reply_status, "이미 전투 중입니다.")
      return
    end
    ...
    battle_reply_status = reply_status
    battle_id =
      if battle_reply_status.is_a?(Hash) && battle_reply_status[:id]
        battle_reply_status[:id].to_s
      else
        "dummy_#{user_id}_#{difficulty}_#{Time.now.to_i}_#{rand(1000)}"
      end
    ...
    BattleState.set(battle_id, state)
    BattleState.assign_user(user_id, battle_id)
  ...
  def attack(user_id, target_id = nil)
    ...
    puts "[디버그] attack 호출: user_id='#{user_id}' (#{user_id.class})"
    puts "[디버그] state 존재: #{!state.nil?}"
    if state
      puts "[디버그] state[:type]=#{state[:type]}"
      puts "[디버그] current_turn='#{state[:current_turn]}' (#{state[:current_turn].class})"
    ...
    if state[:type] == "2v2"
      # 2:2 전투에서는 타겟 필수
      unless target_id
        reply_to_battle_thread("2:2 전투에서는 [공격/@타겟] 형식으로 타겟을 지정해야 합니다.", state)
        return
      end
  ...
  def defend(user_id, target_id = nil)
    ...
    if state[:type] == "2v2"
      # 2:2 전투에서 타겟 지정 방어
      if target_id
        defend_target(user_id, target_id)
      else
        # 자신 방어
  ...
  def defend_target(user_id, target_id)
    ...
    unless target_user
      reply_to_battle_thread("대상을 찾을 수 없습니다.", state)
      return
    end

    unless state[:participants].include?(target_id)
      reply_to_battle_thread("전투 참가자가 아닙니다.", state)
      return
    end

    if state[:type] == "2v2"
      handle_2v2_action(user_id, :defend_target, target_id, state)
      return
    end

    # 1:1 전투에서는 타겟 방어 불가
    reply_to_battle_thread("1:1 전투에서는 [방어]만 사용할 수 있습니다.", state)
  end
  ...
  def process_2v2_round(state, prefix_message)
    ...
    battle_id = state[:battle_id] || BattleState.battle_of(state[:participants].first)

    if team1_alive == 0
      message += "팀2 승리!"
      reply_to_battle_thread(message, state)
      BattleState.clear(battle_id) if battle_id
      return
    elsif team2_alive == 0
      message += "팀1 승리!"
      reply_to_battle_thread(message, state)
  ...
  def check_critical_hit(luck)
    ...
    if roll <= crit_chance
      { is_crit: true, roll: roll, chance: crit_chance }
    else
      { is_crit: false, roll: roll, chance: crit_chance }
    end
  end

  def perform_player_attack(attacker_id, attacker, defender_id, state)
    defender = @sheet_manager.find_user(defender_id)
    battle_id = state[:battle_id] || BattleState.battle_of(attacker_id)
  ...
  def perform_player_attack_on_dummy(user_id, attacker, state)
    ...
    battle_id = state[:battle_id] || BattleState.battle_of(user_id)
