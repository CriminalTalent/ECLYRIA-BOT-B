...
class BattleEngine
  ...
  # ✅ 모니터 스레드 중복 생성 방지 (프로세스 내 1개만)
  @@time_monitor_started = false

  def initialize(client, sheet_manager)
    @client = client
    @sheet_manager = sheet_manager
    start_time_monitor_once
  end
  ...
  # ✅ 시간 제한 모니터링 스레드 (중복 방지)
  def start_time_monitor_once
    return if @@time_monitor_started
    @@time_monitor_started = true
  ...
  def check_all_battles_timeout
    BattleState.get_all_battles.each do |battle_id, state|
      ...
      # 전체 전투 시간 제한 체크 (1시간)
      if state[:battle_start_time] && current_time - state[:battle_start_time] > BATTLE_TIME_LIMIT
        end_battle_by_hp_total(battle_id, state)
        next
      end

      # 턴 시간 제한 체크 (4분)
      if state[:last_action_time] && current_time - state[:last_action_time] > TURN_TIME_LIMIT
  ...
  def auto_defend_team_battle(battle_id, state)
    ...
    execute_round(battle_id) if all_players_ready?(state)
  end
  ...
  def calculate_team_hp(team_members)
    team_members.sum { |member_id| get_user_hp(member_id) }
  end
  ...
  # ✅ 참가자 멘션 문자열 만들기 (@id @id ...)
  def participants_mentions(user_ids)
    user_ids.map { |uid| "@#{uid}" }.join(' ')
  end

  # ✅ 타입별 행동 안내문
  def actions_hint_for(type)
    if type == "pvp"
      "[공격] [방어] [반격] [물약사용/크기] [도주]"
    else
      "[공격/@타겟] [방어/@타겟] [반격] [물약사용/크기/@타겟] [도주]"
    end
  end
  ...
  def start_battle(user1_id, user2_id, reply_status)
    if BattleState.find_by_participant(user1_id)
      @client.reply(reply_status, "#{get_user_name(user1_id)}님은 이미 전투 중입니다.")
      return
    end

    if BattleState.find_by_participant(user2_id)
      @client.reply(reply_status, "#{get_user_name(user2_id)}님은 이미 전투 중입니다.")
      return
    end
    ...
    first_turn_agi = agi1 >= agi2 ? agi1 : agi2

    # ✅ 참여자 태그 + 1:1 전용 버튼
    mention_line = participants_mentions([user1_id, user2_id])

    message = "#{mention_line}\n"
    message += "━━━━━━━━━━━━━━━━━━\n"
    message += "전투 시작: #{user1_name} vs #{user2_name}\n"
    message += "선공: #{first_turn_name} (민첩 #{first_turn_agi})\n"
    message += "━━━━━━━━━━━━━━━━━━\n"
    message += "제한시간: 1인당 4분, 전체 1시간\n"
    message += "━━━━━━━━━━━━━━━━━━\n"
    message += "#{first_turn_name}의 차례\n"
    message += actions_hint_for("pvp")
  ...
  def start_2v2_battle(team1, team2, reply_status)
    ...
    participants.each do |id|
      if BattleState.find_by_participant(id)
        @client.reply(reply_status, "#{get_user_name(id)}님은 이미 전투 중입니다.")
        return
      end
    end
    ...
    mention_line = participants_mentions(participants)
  ...
  def register_action(user_id, action_type, target_id, battle_id, extra_data = nil)
    ...
    state[:last_action_time] = Time.now
    state[:actions] ||= {}
    state[:actions][user_id] = { action: action_type, target: target_id, extra: extra_data }
    BattleState.update(battle_id, state)

    user = @sheet_manager.find_user(user_id)
    user_name = user ? (user["이름"] || user_id) : user_id
    action_text = format_action_text(action_type, target_id, extra_data, user_name, user_id)
  ...
  # ✅ defend 비교 버그 수정 포함
  def format_action_text(action_type, target_id, extra_data, user_name, user_id)
    case action_type
    when :attack
  ...
  def handle_team_action(battle_id, state, action_text)
    ...
    if all_players_ready?(state)
      ...
      message += actions_hint_for(state[:type])
