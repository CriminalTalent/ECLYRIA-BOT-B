# mastodon_client.rb
# 스트리밍 연결 안정화 + 멘션 정규화 + FrozenError 완화(메시지 dup) 버전
...
class MastodonClient
  ...
  def stream(&block)
    ...
    loop do
      begin
        ...
        # PM2 stop/restart 등으로 시그널이 오면 여기로 들어올 수 있음
        puts "[MastodonClient] 스트리밍 종료 신호 감지: #{e.class} - #{e.message}"
        raise
        ...
        if retry_count <= max_retries
          sleep_time = [retry_count * 2, 60].min
          puts "[MastodonClient] #{sleep_time}초 후 재연결 시도..."
          sleep(sleep_time)

          # 새로운 스트리밍 클라이언트 생성(가능하면 base_url/token을 원본에서 유지)
          begin
            # mastodon-api 버전에 따라 base_url/bearer_token 접근자가 없을 수 있으니 안전하게 처리
            base = @streaming.respond_to?(:base_url) ? @streaming.base_url : nil
            token = @streaming.respond_to?(:bearer_token) ? @streaming.bearer_token : nil

            if base && token
              @streaming = Mastodon::Streaming::Client.new(base_url: base, bearer_token: token)
            else
              # 접근자 없으면 기존 값으로 재생성 시도(최악의 경우 그대로 재시도)
              # base_url/token을 저장해두는 구조면 더 안정적이지만, 기존 인터페이스 유지 위해 최소 변경.
            end
          rescue => init_error
  ...
  def reply(status, message)
    # FrozenError 완화: message를 새 문자열로 복제
    safe_message = message.to_s.dup
    ...
    begin
      ...
      begin
        # 재시도도 안전 문자열로
        safe_message2 = safe_message.to_s.dup
        result = @client.create_status(
          safe_message2,
          in_reply_to_id: status_id,
          visibility: visibility
  ...
  def post(message, visibility: 'public')
    ...
    begin
      result = @client.create_status(safe_message, visibility: visibility)
  ...
  def reply_battle_thread(to_status, main_message, status_message = nil, participants = [])
    ...
    if participants.any?
      mention_text = participants.map do |p|
        p = p.to_s.strip
        p = p.sub(/\A@/, '') # 맨 앞 @만 제거(도메인 @는 유지)
        "@#{p}"
      end.join(" ")
    ...
    messages.each do |msg|
      ...
      if msg.length <= 450
        ...
        final_messages.concat(split_message_by_lines(msg, 450))
      end
    end
  ...
  def reply_with_mentions(status, message, mention_users = [])
    if mention_users.any?
      mention_text = mention_users.map do |user|
        u = user.to_s.strip.sub(/\A@/, '')
        "@#{u}"
      end.join(" ")
